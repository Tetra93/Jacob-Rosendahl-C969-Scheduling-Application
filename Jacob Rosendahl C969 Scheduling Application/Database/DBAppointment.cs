...
namespace Jacob_Rosendahl_C969_Scheduling_Application.Database
{
    class DBAppointment
    {
        ...
        {
            CheckAppointment(-1);
            DBConnection.SqlString = $"INSERT INTO appointment (appointmentId, customerId, userId, title, description, location, contact, type, url, start, end, createDate, createdBy, lastUpdate, lastUpdateBy)" +
                $" VALUES ({AppointmentID}, {AddUpdateAppointments.CustomerID}, {AddUpdateAppointments.UserID}, \"N/A\", \"N/A\", \"N/A\", \"N/A\", \"{AddUpdateAppointments.AppointmentType}\", \"N/A\", \"{AddUpdateAppointments.StartTime.ToUniversalTime().ToString("yyyy-MM-dd H:mm:ss")}\", \"{AddUpdateAppointments.EndTime.ToUniversalTime().ToString("yyyy-MM-dd H:mm:ss")}\", CURRENT_TIMESTAMP(), \"{Login.UserName}\", CURRENT_TIMESTAMP(), \"{Login.UserName}\")";
            DBConnection.Cmd = new MySqlCommand(DBConnection.SqlString, DBConnection.Conn);
